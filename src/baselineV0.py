...
def single_model(clf, train_x, train_y, test_x, clf_name, class_num=1):
    ...
    if clf_name == "xgb":
        ...
        test_pred = 0#model.predict(test_matrix , ntree_limit=model.best_ntree_limit).reshape(-1,1)
    ...
    if clf_name == "cat":
        ...
        test_pred = 0#model.predict(test_x)
    ...
    print("%s_mbe_score:" % clf_name, mean_absolute_error(val_y, val_pred))
...
nums = int(train_df.shape[0] * 0.70)
y_val = y_train[nums:]
...
print('stacking val mse:', mean_squared_error(y_val,train_pred))
